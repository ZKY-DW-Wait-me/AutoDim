...
> ## ⚠️ 开发路线更正（2026-08）
>
> 此前若干次推送把"扫描碎片图清洗"当成了主路线，默认参数面向扫描件
> （0.5mm 网格化取整、过滤小特征、合并碎片），对 **SolidWorks / CAD 导出的干净工程图**
> 做了破坏性处理——把精确坐标取整、把圆角圆弧圆心搞漂移、过滤掉小特征，导致
> "抄一遍抄坏了"的观感。**这是方向性错误，向使用者道歉。**
>
> 正确路线已纠正：
> - 默认参数改为**面向干净 CAD 图**：吸附公差 0.01（保留 SW 精确坐标）、不过滤小特征；
>   扫描碎片件才需要用 `ADIMCLN` 调紧（SnapTol=0.5 等）。
> - 修复了圆弧标注的三层几何 bug（面提取 bulge 错位、面追踪按弦方向转向、清洗取整
>   导致圆心漂移），圆头圆弧现在能正确标出 R6 而非 R5.69/R6.42。
> - 尺寸避让改为按"文字框"判据，不再把不撞文字的标注推出图纸外 60mm。
>
> 下一步专注：**标注质量本身**——像人工标注那样符合国标的图（标什么、尺寸线怎么排、
> 文字怎么放、孔怎么标），而不是折腾图纸清洗。
