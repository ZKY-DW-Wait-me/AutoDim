...
| 能力 | 3B 实测 | 结论 |
|------|---------|------|
| 合成图"有无噪声"二分类 | 4/4 对 | ✅ 可用 |
| 圆孔计数（合成图 5/12 个） | 答 1/6 | ❌ 不可用 |
| 真实图纸线条密度（清洗前 2.32% vs 后 0.37% 墨迹） | 均答"少" | ❌ 无区分度 |
| 标注质量分级（密集/稀疏区域） | 均答"有重叠/一般" | ❌ 无区分度 |

结论：3B 视觉模型对 CAD 密集线框图的细粒度理解不足，**只能做最简单的"有无噪声"二分类**。
7B（qwen2.5vl:7b，~5GB）视觉能力跨量级，待同脚本对比：
`python test/mllm_quality_test.py qwen2.5vl:7b`。
...
去重圆              447
特征组              136
自动标注总量        344 个尺寸 + 61 个 N×Ød 注记
ADIM 文字撞车对数   5（真实文字外接框相交，可读性指标）
无界面运行耗时      约 17s
```
